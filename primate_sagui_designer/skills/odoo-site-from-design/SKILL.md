...
- **Type — define an EXPLICIT scale, don't eyeball it.** Set exact `font-family`, size,
  ...
  them via Google Fonts. Choose a **type pair with character, derived from the reference**: read
  what the reference's letterforms actually are (condensed or wide, grotesque or humanist, high
  or low contrast) and match that instead of reaching for a default. A poster-style condensed
  display wants a tall condensed grotesque; a handwritten aside wants a script face. **If the
  reference does not define a typeface at all** (a wireframe, a greyscale layout, a mockup set in
  system text), do NOT fall back to a neutral default — choose the pair the way greenfield
  Stage 1 does, deriving it from the subject, and state the choice. Type is most of the
  personality — getting the family, weight and scale right does more for fidelity than any other
  single change.
...
- **Selector discipline — this is what breaks the seams between blocks.** Every property is
  declared at ONE level of specificity. Vertical rhythm belongs to the section rule and nowhere
  else (`.brandsite .sec { padding-block: var(--space-xl) }`); a per-section rule
  (`.brandsite .cta`) must never restate padding that `.sec` already owns, and element selectors
  must never fight class selectors over the same property. When two rules of different specificity
  set the same property, spacing between blocks collapses or doubles unpredictably — that is the
  "mal ensamblado" look. Per-section rules are for internal layout only.

### Pass 3 — Visual verification against the reference (not a self-critique)
Reading your own HTML and judging it does not work: the agent believes what it wrote. The textual
self-critique is replaced by the **visual verifier**:

1. Render the real Odoo page (Playwright) at 375, 768 and 1440 px, full page — anonymous session,
   plus one logged-in capture.
2. Send the screenshots + the extracted design system to a **reviewer model from a different
   provider than the generator** (Sagui's cross-review rule), with
   `odoo-site-greenfield-design/references/verifier-rubric.md` as the rubric. The reviewer never
   sees the generated HTML: it judges what is visible.
3. The reviewer returns concrete findings only — rubric item, section, breakpoint, and the
   smallest change that fixes it. "Improve the design" is rejected.
4. Apply fixes. Size, spacing, color and weight are a **deterministic edit of the token block**,
   never a regeneration. Regenerate a single section only for layout/structure failures.
5. Re-shoot, re-review. Stop when the rubric passes or after 3 loops, then report what is left.

Some rubric items are established against the **DOM**, not the screenshot — the admin edit bar
(`.o_frontend_to_backend_nav`) ships with `d-none` and is only revealed by `redirect.js`, so a PNG
proves nothing about it. When a finding was established that way, the report says so instead of
implying it was seen in an image.

The checklist below stays as the build-time contract; the verifier is what confirms it.
...
- [ ] No AI-default tells. The full list, shared with `odoo-site-greenfield-design`, is
      `_shared/anti-patterns.md` — every tell with the reason it reads as templated and what to do
      instead. Read it; do not rely on remembering it.
